...
namespace Lboto.Helpers.Tasks
{
    ...
    {
        #region temp
        private int TempMaxLootDinstanceSetting = 120;
        #endregion

        private const int MaxItemPickupAttempts = 3;
        ...
        public async Task<bool> Run()
        {
            ...
            var refPos = LokiPoe.Me.Position;

            validItems = allItems.FindAll(i => i.Position.AsVector.Distance(refPos) <= TempMaxLootDinstanceSetting);
            ...
            if (validItems.Count == 0)
            {
                if (itToFarCount > 0)
                {
                    Log.Warn($"[LootItemTask] {itToFarCount} will be ignored B/C they are to far away. (Increase MaxLootDistance or bring the Leader closerto them.)");
                }
                return false;
            }
            ...
            {
                Log.Warn($"[LootItemTask] No room in inventory for {_item.Position.Name}");
                _item.Ignored = true;
                _item = null;
                return true;
            }
            ...
            if (pos.Distance > TempMaxLootDinstanceSetting)
            {
                Log.Warn($"[LootItemTask] {pos} is now to far away. [{pos.Distance}/{TempMaxLootDinstanceSetting}]");
                _item.Ignored = true;
                _item = null;
                return true;
            }
            if (pos.Distance > 30 || pos.PathDistance > 34)
            {
                ...
                {
                    Log.Debug($"[LootItemTask] Items to pick up: {validItems.Count}");
                    Log.Debug($"[LootItemTask] Moving to {pos}");
                }
                ...
                if (!PlayerMoverManager.MoveTowards(pos))
                {
                    ...
                    {
                        Log.Error($"[LootItemTask] Fail to move to {pos}. Marking this item as unwalkable.");
                        _item.Unwalkable = true;
                        _item = null;
                    }
                    else
                    {
                        Log.Error($"[LootItemTask] Fail to move to {pos}. item Object is null, removing item from the cache and reevaluating it.");
            ...
            if (attempts > MaxItemPickupAttempts)
            {
                ...
                {
                    Log.Error("[LootItemTask] All attempts to pick up an item have been spent. Now ignoring it.");
                    _item.Ignored = true;
                    _item = null;
                }
                return true;
            ...
            {
                Log.Debug($"[LootItemTask] skipping {pos}, becouse it don't have a visible label");
                _item.Ignored = true;
                _item = null;
                return true;
            }

            Log.Debug($"[LootItemTask] Now picking up {pos}");
            ...
            if (await FastInteraction(itemObj))
            {
                ...
                if (await Wait.For(() => _item.Object == null, "item pick up", 5, timeout))
                {
                    items.Remove(_item);
                    _item = null;
                    Log.Info($"[Events] Item looted ({cached.Name})");
                    //Utility.BroadcastMessage(this, Events.Messages.ItemLootedEvent, cached); not yet implimented
                }
                return true;
            }
            return true;
        ...
        private static async Task<bool> FastInteraction(WorldItem item)
        {
            ...
            {
                //Log.Info($"[FastInteraction] pressing UseHighlightKey Key [{LokiPoe.Input.Binding.highlight_combo.Modifier} + {LokiPoe.Input.Binding.highlight_combo.Key}]");
            ...
            {
                //Log.Info($"[FastInteraction] pressing UseBoundKey [{LokiPoe.Input.Binding.enable_key_pickup_combo.Modifier} + {LokiPoe.Input.Binding.enable_key_pickup_combo.Key}]");
        ...
        private static async Task<bool> MoveAway(int min, int max)
        {
            ...
            if (pos == null)
            {
                Log.Debug("[LootItemTask] Fail to find any pathable position at distance.");
                return false;
            }
