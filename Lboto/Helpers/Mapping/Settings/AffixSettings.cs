...
namespace Lboto.Helpers.Mapping
{
    public class AffixSettings
    {
        ...
        private static readonly string AffixSettingsFilePath;
        private static AffixSettings _affixSettings;        
        private readonly List<AffixData> _AffixList = new List<AffixData>();        
        private readonly Dictionary<string, AffixData> dictionary_0 = new Dictionary<string, AffixData>();
        public static AffixSettings Instance => _affixSettings ?? (_affixSettings = new AffixSettings());
        ...
        private AffixSettings()
        {
            ...
            _AffixList = (from a in AffixList
        ...
        private void Load()
        {
            if (!File.Exists(AffixSettingsFilePath))
            {
                return;
            }
            string text = File.ReadAllText(AffixSettingsFilePath);
            if (!string.IsNullOrWhiteSpace(text))
            {
                ...
                Log.Error("[MapBot] Fail to load \"AffixSettings.json\". Json deserealizer returned null.");
            }
            else
            {
                Log.Error("[MapBot] Fail to load \"AffixSettings.json\". File is empty.");
            }
        }
        ...
        private void Save()
        {
            ...
            File.WriteAllText(AffixSettingsFilePath, contents);
        }

        static AffixSettings()
        {
            AffixSettingsFilePath = Path.Combine(Configuration.Instance.Path, "MapBot", "AffixSettings.json");
        }
    }
}
