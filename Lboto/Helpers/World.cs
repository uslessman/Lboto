    ...
    public static class World
    {
            ...
            foreach (var act in typeof(World).GetNestedTypes())
            {
                foreach (var field in act.GetFields())
                {
                    ...
                    if (areaDict.TryGetValue(area.Id, out var name))
                    {
                        ...
                        {
                            area.Name = name;
                            Log.Error($"[World] Invalid area info in \"{field.Name}\" field. Area name: \"{area.Name}\". Correct name: \"{name}\".");
                            //error = true;
                        }
                    }
                    else
                    {
                        Log.Error($"[World] Invalid area info in \"{field.Name}\" field. DatWorldAreas does not contain an area with \"{area.Id}\" id.");
                        error = true;
                    }
                }
            }
        ...
        public static DatWorldAreaWrapper LastOpenedAct
        {
            get
            {
                ...
                if (act == null)
                {
                    Log.Error("[GetLastOpenedAct] Unknown error. Fail to get any opened act.");
                    return null;
                }
                return act;
            }
