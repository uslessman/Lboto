...
namespace Lboto.Helpers.Global
{
    public static class TrackMobLogic
    {
        ...
        public static async Task<bool> Execute(int range = -1)
        {
            ...
            {
                Log.Debug("[TrackMobLogic] Current target is in global blacklist. Now abandoning it.");
            ...
            if (pos.IsFar || pos.IsFarByPath)
            {
                ...
                {
                    Log.Debug($"[TrackMobTask] Cached monster locations: {cachedMonsters.Valid().Count()}");
                    Log.Debug($"[TrackMobTask] Moving to {pos}");
                }
                if (!PlayerMoverManager.MoveTowards(pos))
                {
                    Log.Error($"[TrackMobTask] Fail to move to {pos}. Marking this monster as unwalkable.");
            ...
            else
            {
                ...
                {
                    Log.Error("[TrackMobTask] All attempts to kill current monster have been spent. Now ignoring it.");
                    CurrentTarget.Ignored = true;
                    CurrentTarget = null;
                    return true;
                }
                Log.Debug($"[TrackMobTask] Alive monster is nearby, this is our {attempts}/{MaxKillAttempts} attempt to kill it.");
