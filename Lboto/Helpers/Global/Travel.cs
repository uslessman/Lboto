...
namespace Lboto.Helpers.Global
{
    public static class Travel
    {
        ...
        public static async Task To(AreaInfo area)
        {
            ...
            {
                Log.Error($"[Travel] Unsupported area: {area}.");
            }
        }
        public static void RequestNewInstance(AreaInfo area)
        {
            ...
            {
                Log.Debug($"[Travel] New instance requested for {area}");
            }
        }
        ...
        {
            Log.Error($"[Travel] Lioneye's Watch waypoint is not opened and we are not inside Twilight Strand. Current area: {World.CurrentArea}");
        ...
        private static async Task Archives()
        {
            ...
            if (World.Act3.Library.IsCurrentArea)
            {
                ...
                if (candle != null)
                {
                    ...
                    {
                        Log.Debug("Waiting for Archives transition.");
                    }
                }
        ...
        private static async Task Mines1()
        {
            ...
            if (World.Act4.Highgate.IsCurrentArea)
            {
                ...
                {
                    Log.Error($"[Travel] Cannot travel to {World.Act4.Mines1}. Deshret Seal is active.");
                    return;
                }
        ...
        private static async Task StrictlyWpAreaHandler(AreaInfo area, string hint, Action postEnter = null)
        {
            if (!area.IsWaypointOpened)
            {
                Log.Error($"[Travel] {area.Name} waypoint is not available. {hint}.");
                return;
            }
            if (AnyWaypointNearby)
            {
        ...
        private static async Task TakeTransition(AreaInfo area, Action postEnter)
        {
            ...
            if (transition == null)
            {
                Log.Error($"[Travel] There is no transition that leads to {area}");
                return;
            }
        ...
        private static async Task<AreaTransition> GetTransitionObject(TgtPosition tgtPos, AreaInfo area)
        {
            ...
            if (transition == null)
            {
                if (area == World.Act5.OverseerTower)
                {
                    ...
                    if (ladder != null)
                    {
                        Log.Debug($"[Travel] Ladder detected");
                    ...
                    Log.Debug($"[Travel] Waiting for \"{area.Name}\" transition activation.");
                    return null;
                }
                Log.Warn("[Travel] There is no area transition near tgt position.");
                tgtPos.ProceedToNext();
                return null;
            }
            if (transition.TransitionType == DreamPoeBot.Loki.Game.GameData.TransitionTypes.NormalToCorrupted)
            {
                Log.Warn("[Travel] Corrupted area entrance has the same tgt as our destination.");
                tgtPos.ProceedToNext();
                return null;
            }
            if (!transition.IsTargetable)
            {
                if (area == null)
                {
                    Log.Debug("[Travel] Waiting for transition activation.");
                }
                else
                {
                    Log.Debug($"[Travel] Waiting for \"{area.Name}\" transition activation.");
            ...
            if (area != null)
            {
                ...
                if (area != dest)
                {
                    Log.Warn($"[Travel] Transition leads to \"{dest.Name}\". Expected: \"{area.Name}\".");
                    tgtPos.ProceedToNext();
                    return null;
                }
            }
        ...
        private static async Task TpToTown()
        {
            if (!await PlayerAction.TpToTown())
                Log.Error("[Travel] failed tp to town");
        }
        ...
        {
            Log.Error($"[Travel] Outer logic error. Travel to {area} has been called, but we are already here.");
        }
