    ...
    public class CombatAreaCache
    {
        ...
        private CombatAreaCache(uint hash)
        {
            ...
            while (World.CurrentArea == null)
            {
                ...
                {
                    Log.Error($"[CombatAreaCache] Failed to Creating cache for area hash: {hash}, World.CurrentArea == null for 5 seconds.");
                    return;
                }
                Thread.Sleep(10);
            }
            Log.Info($"[CombatAreaCache] Creating cache for \"{World.CurrentArea.Name}\" (hash: {hash})");
            Hash = hash;
        ...
        private static void RemoveOldCaches()
        {
            ...
            {
                Log.Info($"[CombatAreaCache] Removing cache for \"{cache.WorldArea.Name}\" (hash: {cache.Hash}). Last accessed {(int)cache._lastAccessTime.Elapsed.TotalMinutes} minutes ago.");
        ...
        private void OnTick()
        {
            ...
            //            }
            //            else
            //                Log.Debug("[CombatAreaCache] The recipe is null.");
            //        }
        ...
        private void ProcessRecipe(CraftingRecipe c)
        {
            ...
            _processedObjects.Add(id);
            Log.Warn($"[CombatAreaCache] Registering Crafting Recipe {pos}");
        }
        private void ProcessSpeacialChest(Chest c)
        {
            ...
            _processedObjects.Add(id);
            Log.Warn($"[CombatAreaCache] Registering {pos}");
        }
        ...
        private void ProcessTransition(AreaTransition t)
        {
            ...
            _processedObjects.Add(id);
            Log.Debug($"[CombatAreaCache] Registering {pos} (Type: {type})");
        ...
        private bool SkipThisTransition(AreaTransition t)
        {
            ...
            {
                Log.Debug($"[CombatAreaCache] Skipping dummy area transition (id: {t.Id})");
                return true;
            }
            if (name == "Tomb" && Current.WorldArea.Name == "Barrows")
            {
                Log.Debug($"[CombatAreaCache] Skipping Tomb area transition (id: {t.Id})");
                return true;
            }
            if ((int)t.TransitionType == (int)TransitionType.Local && !t.Metadata.Contains("IncursionPortal"))
            {
                ...
                {
                    Log.Debug($"[CombatAreaCache] Skipping \"{name}\" area transition because it leads to the same level.");
                    return true;
                }
                if (WorldArea.Id == World.Act9.RottingCore.Id)
                {
                    ...
                    {
                        Log.Debug($"[CombatAreaCache] Skipping \"{name}\" area transition because it is unlocked by a quest.");
                        return true;
                    }
                    if (metadata.Contains("BellyArenaTransition"))
                    {
                        Log.Debug($"[CombatAreaCache] Skipping \"{name}\" area transition because it is not a pathfinding obstacle.");
                        return true;
                    }
                }
            }
        ...
        private void TweakTransition(CachedTransition t)
        {
            ...
            {
                Log.Debug("[CombatAreaCache] Marking this area transition as unwalkable (Villa tweak)");
                t.Unwalkable = true;
                return;
            }
            if (areaName == MapNames.Summit && name == MapNames.Summit)
            {
                Log.Debug("[CombatAreaCache] Marking this area transition as back transition (Summit tweak)");
                t.LeadsBack = true;
            }
        }
        ...
        {
            Log.Info("[CombatAreaCache] Resetting unwalkable flags on all cached objects.");
        ...
        private static void OnItemEvaluatorRefresh(object sender, ItemEvaluatorRefreshedEventArgs args)
        {
            ...
            {
                Log.Info("[CombatAreaCache] Clearing processed items.");
        ...
        public class ObjectDictionary
        {
            ...
            public object this[string key]
            {
                ...
                set
                {
                    ...
                    {
                        Log.Debug($"[Storage] Registering [{key}] = [{value ?? "null"}]");
                        _dict.Add(key, value);
                    }
                    else
                    {
